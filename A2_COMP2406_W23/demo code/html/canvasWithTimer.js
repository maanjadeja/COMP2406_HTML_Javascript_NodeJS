...
function getWidthofWord(word) {
	const context = canvas.getContext('2d');
	context.font = '20pt Arial';
	return context.measureText(word).width;
}

function randomizeLocationOfWords() {
  for (let i = 0; i < words.length; i++) {

    // <canvas id='canvas1' class='canvas' width='600' height='300'>
    

    //calculate random values for the x and y coordinate of the words
    // let xCoordinate = Math.floor(Math.random() * 600)
    // let yCoordinate = Math.floor(Math.random() * 300)

    // while((xCoordinate>600) || (yCoordinate>300)){
    //   xCoordinate = Math.floor(Math.random() * 600)
    //   yCoordinate = Math.floor(Math.random() * 300)
    // }

    //Set the random coordinates the coordinates of the words
    words[i].x = getRandomNumber(0,canvas.width-getWidthofWord(words[i].word));
    words[i].y = getRandomNumber(24, canvas.height);
...
function getWordAtLocation(aCanvasX, aCanvasY) {
  ...
  console.log("aCanvasX: " + aCanvasX);
  console.log("aCanvasY: " + aCanvasY);

  let TOLERANCE = 0;

  for (let i = 0; i < words.length; i++) {

    console.log("words[i].length: " + words[i].word.length);
    // words[i].word.stringWidth = context.measureText(words[i].word).width
    TOLERANCE = (context.measureText(words[i].word).width);


    // let TOLERANCE = words[i].word.length*20;

    if (aCanvasX - words[i].x >= 0 && aCanvasX - words[i].x < TOLERANCE &&
      words[i].y - aCanvasY >= 0 && words[i].y - aCanvasY < 24) return words[i]
  }
  return null
}
