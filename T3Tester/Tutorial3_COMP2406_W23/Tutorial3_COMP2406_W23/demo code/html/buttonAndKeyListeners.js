    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (responseObj.text.includes("NOT FOUND") === false) {
          ...
          let allSongWords = JSON.stringify(responseObj.songLines[0])

          allSongWords = allSongWords.replace(/['"]+/g, '');

          let songArray = allSongWords.split(" ") //create an array of all the words seperated by " "
          console.log("SONGARRAY: " + songArray)
          words = []; //reset the words array to empty
          for (let i = 0; i < songArray.length; i++) { //loop through the array
            let songArrayObject = { word: songArray[i], x: i * 10, y: i * 20 } //create the object to add into words[] array
...
function handleSubmitWithFetch() {
