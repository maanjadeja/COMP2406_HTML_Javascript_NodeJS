      ...
      if(exists){
        ...
        fs.readFile(songFile, function(err, data) {
          ...
          if (err) {
            ...
            for(let x=0; x<returnObj.songLines.length; x++){
              for(let y=0; y<returnObj.songLines[x].length; y++){
               console.log("LETTER: ")
               console.log(returnObj.songLines[x][y]);
              }
            }
            // console.log(returnObj.songLines[0]);

            // console.log("fileLines: "+returnObj.songLines);
            /*
            let songArray = JSON.stringify(returnObj.songLines).split(" ");
            for(let k=0; k<songArray.length; k++){
              for(let m=0; m<songArray[k].length; m++){
                if(songArray[k][m]==="[" || songArray[k][m]==="]" || songArray[k][m]==='"' ){
                  let startingIndex = songArray[k][m].indexOf(/A-Za-z/);
                  let endingIndex = songArray[k][m].length;
                  
                  
                  songArray[k][m] = songArray[k][m].substring(startingIndex,endingIndex);
            ...
            console.log(JSON.stringify(returnObj.songLines).split(" "))
            console.log(JSON.stringify(returnObj.songLines).split(" ")[0])
            console.log(JSON.stringify(returnObj.songLines).split(" ")[1])

            console.log("THE SONG LINES: "+typeof(JSON.stringify(returnObj.songLines).split(" ")))

            */
            response.writeHead(200, { "Content-Type": MIME_TYPES["json"] })
            // let x = JSON.stringify(returnObj.songLines);
            // x.replace("[","")
            // console.log("x = "+typeof(x));
            // console.log(x);
            // console.log("returnObj as JSON.stringify()"+JSON.stringify(returnObj));
            response.end(JSON.stringify(returnObj))
          }
        })
      }
